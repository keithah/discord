...
func newHeartbeatOp(seq int64) heartbeatOp {
	return heartbeatOp{
		Op:   1,
		Data: seq,
	}
...
func (s *Session) heartbeat(ctx context.Context, wsConn *websocket.Conn, listening <-chan interface{}, heartbeatIntervalMsec time.Duration) {
	...
	for {
		...
		heartbeat := any(newForegroundedQosHeartbeatOp(sequence))
		if !s.IsUser {
			heartbeat = newHeartbeatOp(sequence)
		}
		s.wsMutex.Lock()
		s.LastHeartbeatSent = time.Now().UTC()
		err = wsjson.Write(ctx, wsConn, heartbeat)
		s.wsMutex.Unlock()
...
func (s *Session) onEvent(messageType websocket.MessageType, message []byte, isOnConnect bool) (*Event, error) {
	...
	if e.Operation == 1 {
		...
		heartbeat := any(newForegroundedQosHeartbeatOp(atomic.LoadInt64(s.sequence)))
		if !s.IsUser {
			heartbeat = newHeartbeatOp(atomic.LoadInt64(s.sequence))
		}
		s.wsMutex.Lock()
		err = wsjson.Write(s.wsConnCtx, s.wsConn, heartbeat)
		s.wsMutex.Unlock()
		if err != nil {
